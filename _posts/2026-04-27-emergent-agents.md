...
To allow different agents to interact and possibly trade with each other, we adopted a time-varying network interaction model inspired by complex networks and Markov chains <d-cite key="watts1998collective"></d-cite>. Let $$ N\in\mathbb{N} $$ be the number of agents and $$ t\in\mathbb{Z}_{\ge 0} $$ the discrete time index. Each agent $$ A_i\in\{1,\dots,N\} $$ produces a directional score for agent $$ A_j $$ via
